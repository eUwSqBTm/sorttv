...
sub sort_directory {
	...
	FILE: foreach my $file (bsd_glob($escapedsortd.'*'), @files) {
		...
		# movie should come last since it will attempt to find a matching movie name on tmdb
			...
			if ($@) {
				out("warn", "WARN: Error sorting $file: ".Dumper($@)."\n");
				if(Dumper($@) =~ /Deep:/) {
...
sub resolve_show_name {
	my ($title) = @_;
	my $test = substitute_name(remdot($title));
...
sub tvdb_title {
	...
	if($tvdbrename eq "TRUE") {
		...
		my $retval;
		eval { $retval = $tvdb->getSeries($filetitle); };
		if ($@) {
    	# warn "Ignore this error: $@"; # there is a problem in the TVDB CPAN module where it has issues with invalid XML, so just skip errors here
		}
...
sub match_and_sort_movie {
	...
	PAGE: for($page_index = 1; $page_index <= $total_pages; $page_index++) {
		...
		MOVIE: while($movie = $$parsed_json_result{"results"}[$result_index]) {
			...
			my $baseimgURL = "https://image.tmdb.org/t/p/w600_and_h900_bestv2/";
	...
	if(sort_file($file, $moviedir.$dest, "MOVIE") eq "TRUE") {
		...
		if($dryrun) {
			...
			out("std", "INFO: Fetching movie images\n");
				...
				for(my $tries=0;$tries<2&&(!$status1||!LWP::Simple::is_success($status1));$tries++) {
					$status1 = LWP::Simple::getstore( $posterimg, $posterfilename );
					$status1 && LWP::Simple::is_success($status1) || out("warn", "Failed to download image\n");
				}
				for(my $tries=0;$tries<2&&(!$status2||!LWP::Simple::is_success($status2));$tries++) {
					$status2 = LWP::Simple::getstore( $backimg, $fanartfilename );
					$status2 && LWP::Simple::is_success($status2) || out("warn", "Failed to download image\n");
				}
				my $ok = 1;
				if (-e $posterfilename) {
					# if configured to symlink
